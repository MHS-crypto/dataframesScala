...
val weatherDataFrame: DataFrame = spark.read
  .format("csv")
  .option("header", "true") // If your CSV file has a header row
...
// The schema matches with the file in the csv.
...
val minTemp: Double = weatherDataFrame.agg(min("temperaturemin")).head().getAs[Double](0)
val maxTemp: Double = weatherDataFrame.agg(max("temperaturemax")).head().getAs[Double](0)
...
val task6DF: DataFrame = weatherDataFrameWithYear
.groupBy("year")
.agg(
 min("temperaturemin").alias("min_temperature"),
 max("temperaturemax").alias("max_temperature"),
 count("year").alias("entries"),
...
val min2012: Double = task6DF
.filter(col("year") === 2012)
.select("min_temperature")
.first()
.getAs[Double]("min_temperature")

val max2016: Double = task6DF
.filter(col("year") === 2016)
.select("max_temperature")
.first()
.getAs[Double]("max_temperature")

val entries2018: Long = task6DF
.filter(col("year") === 2018)
.select("entries")
.first()
.getAs[Long]("entries")

val wind2008: Double = task6DF
.filter(col("year") === 2008)
.select("avg_windspeed")
.first()
.getAs[Double]("avg_windspeed")
...
val fogDaysByYear = weatherDataFrameWithYear
  .groupBy("year")
        .agg(
    count(when(col("fog") === "Yes", 1)).alias("numFogDays")
  )

val yearWithMostDaysWithFog: Int = fogDaysByYear
  .orderBy(desc("numFogDays"))
  .select("year")
  .first()
  .getAs[Int]("year")
